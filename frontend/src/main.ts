...
function renderSettings() {
  ...
  const c = config;
  app.innerHTML = `<div class="shell"><aside><div class="brand" data-window-drag><strong>D</strong><div><b>豆包语音输入</b><small>VOICE UTILITY</small></div></div><nav aria-label="设置分类"><button class="active" data-page="general">常规设置 <small>GENERAL</small></button><button data-page="hotkeys">热键配置 <small>HOTKEYS</small></button><button data-page="floating">悬浮窗口 <small>HUD</small></button><button data-page="asr">识别引擎 <small>ASR</small></button><button data-page="cloud">云端增强 <small>CLOUD</small></button></nav><div class="ready"><i></i>服务已就绪<br/><small>双击 Ctrl 开始输入</small></div></aside><main class="settings-main"><header data-window-drag><div><p>SETTINGS / 控制台</p><h1 id="page-title">常规设置</h1></div><div class="header-actions"><button class="primary" id="save">保存更改</button><button class="window-close" id="close" title="关闭" aria-label="关闭">×</button></div></header><div class="page-stack">
