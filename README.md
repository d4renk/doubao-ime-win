...
- ☁️ **云端增强可控** - 可分别关闭异步实体识别与会话结束后的自动润色
...
- 语音会话结束后后台获取完整润色结果，只有用户确认“替换”时才一次性写回目标应用。
- 使用 UI Automation 限量读取光标前后文，并在替换前恢复和校验原目标窗口。
- 新增 NER、自动润色开关以及完整结果确认窗口。
...
auto_polish_enabled = true  # 会话结束后请求完整润色结果
```

启用云端增强时，最终识别文本会发送到兼容服务。NER 响应不会修改或展示在当前输入框中；自动润色只展示完整结果，必须由用户点击“替换”后才会一次性写回目标应用。可在设置窗口或 `[cloud]` 配置中分别关闭这两项上传。
