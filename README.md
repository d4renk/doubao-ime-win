...
- ☁️ **云端增强可控** - 可分别关闭异步实体识别与会话结束后的 3 秒口水词清理
...
auto_polish_enabled = true  # 会话结束后在 3 秒内清理口水词并自动替换
```

启用云端增强时，最终识别文本会发送到兼容服务。NER 响应不会修改或展示在当前输入框中；口水词清理只删除口头语、语气词、重复和无意义停顿，不扩写。完整结果在 3 秒总预算内返回时会自动一次性替换 ASR 原文；超时或失败时保留原文。可在设置窗口或 `[cloud]` 配置中分别关闭这两项上传。
