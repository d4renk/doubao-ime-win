...
auto_polish_enabled = true  # 会话结束后流式清理，10 秒内完成则自动一次性替换
```

启用云端增强时，最终识别文本会发送到兼容服务。NER 响应不会修改或展示在当前输入框中；口水词清理只删除口头语、语气词、重复和无意义停顿，不扩写。LLM 响应以 SSE 流式接收，delta 仅在内存中聚合；10 秒内收到完整结果时自动一次性替换 ASR 原文，超时或失败时保留原文。可在设置窗口或 `[cloud]` 配置中分别关闭这两项上传。
